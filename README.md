...
FirstLine client project created from the v2.3 starter. It is intentionally small: static HTML/CSS/JS, reusable navigation/anchor/footer/form utilities, UX Pilot design-source project memory, and Playwright checks for issues that repeatedly appeared in historical projects.
...
Place the raw UX Pilot HTML export in:

```text
design-source/uxpilot/
```

The raw export is immutable project reference material. Do not overwrite it during implementation. Track the selected export, import status, intended visual features, material deviations, and open visual questions in `docs/DESIGN_SOURCE.md`.

UX Pilot HTML is a design specification, not necessarily production code. FirstLine may refactor inline styles, style tags, generated utility classes, external CSS, inline/external scripts, inline event handlers, embedded SVG, asset URLs, class names, semantics, and accessibility structure into maintainable project files while preserving rendered visual fidelity.

Run the canonical system workflow from the client repo after adding the export:

```powershell
Build-FirstLineFromUXPilot
```
...
- `docs/DESIGN_SOURCE.md`: UX Pilot design-source tracking, import status, and material deviations.
- `docs/FIRSTLINE_LEARNING.md`: candidate reusable lessons found during the project.
- `design-source/uxpilot/`: preserved raw UX Pilot HTML exports.
...
Replace placeholder content and tokens with the client brief and UX Pilot design source before build work. Do not use the starter's sample layout or starter CSS as the art direction. Do not promote project-specific changes into FirstLine-System automatically; record candidates in `docs/FIRSTLINE_LEARNING.md`.
